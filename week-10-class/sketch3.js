...
function draw() {
  ...
  let posx = map(inData, 0, 1023, 1, 255);
  let posy = map(inData2, 0, 1023, 1, 255);
  let hue = parseInt(posx);
  let sat = parseInt(posy);

  background(color(hue, sat, 100));
  console.log(hue, sat);

  let colorText = text(`hue: ${hue} \nsat: ${sat}`, 150, 200);
  colorText.fill("white");
  colorText.textSize(40);
}
